...
namespace Wanted
{
	...
	void Renderer::Draw()
	{
		...
		for (const RenderCommand& command : renderQueue)
		{
			...
			int screenX = static_cast<int>(command.position.x - cameraPosition.x);
			int screenY = static_cast<int>(command.position.y - cameraPosition.y);
			...
			// 위치는 왼쪽 기준: "abcde"
			const int startX = screenX;
			const int endX = screenX + length - 1;
