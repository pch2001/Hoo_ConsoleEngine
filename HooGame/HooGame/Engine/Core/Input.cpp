			...
			if (ReadConsoleInput(inputHandle, records, recordCount, &eventReadCount))
			{
				...
				for (int ix = 0; ix < static_cast<int>(eventReadCount); ++ix)
				{
					...
					case MOUSE_EVENT:
					{
						...
						mousePosition.x = Util::Clamp<int>(mousePosition.x, 0, Engine::Get().GetWidth() - 1);

						mousePosition.y = record.Event.MouseEvent.dwMousePosition.Y;
						// y좌표 조정.
						mousePosition.y = Util::Clamp<int>(mousePosition.y, 0, Engine::Get().GetHeight() - 1);

						// 마우스 클릭 여부 상태 저장.
						keyStates[VK_LBUTTON].isKeyDown	= (record.Event.MouseEvent.dwButtonState & FROM_LEFT_1ST_BUTTON_PRESSED) != 0;

						keyStates[VK_RBUTTON].isKeyDown	= (record.Event.MouseEvent.dwButtonState & RIGHTMOST_BUTTON_PRESSED) != 0;
					}
					break;
					}
				}
